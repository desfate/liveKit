...
//        Optional.ofNullable(listener).ifPresent(onDrawTexture -> onDrawTexture.onDrawTexture(texture,1080, 1920));   //  在这调用接口  预览绘制异常  结束直播后恢复正常
...
//        Optional.ofNullable(listener).ifPresent(onDrawTexture -> onDrawTexture.onDrawTexture(texture,1080, 1920));   //  在这调用接口  预览黑屏   结束直播后恢复正常
        // 真正绘制的操作
...
//        Optional.ofNullable(listener).ifPresent(onDrawTexture -> onDrawTexture.onDrawTexture(texture,1080, 1920)); //  调用接口  预览正常
    }
...
//            System.out.println(op + ": glError " + error);
        }
    }
}
